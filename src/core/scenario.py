...
def mixed_battle_scenario(general_a, general_b):
    """Teste les contres : 
    Armée A : ligne de pikmens + ligens archers + carré chevalier
    Armée B : carré chevaliers"""
    ...
    world_map = Map(width=120 * TILE, height=120 * TILE)
    all_units = []

    # Ligne de Piquiers au centre
    spawn_line(player_a, world_map, all_units, UnitType.PIKEMAN, 50*TILE, 50*TILE, 10, TILE)
    # Ligne d'Archers juste derrière
    spawn_line(player_a, world_map, all_units, UnitType.ARCHER, 50*TILE, 55*TILE, 8, TILE)
    # Groupe de Chevaliers sur le côté
    spawn_square(player_a, world_map, all_units, UnitType.KNIGHT, 40*TILE, 50*TILE, 2, 2, TILE, 4)

    spawn_square(player_b, world_map, all_units, UnitType.KNIGHT, 70*TILE, 50*TILE, 5, 6, TILE, 30)

    return [player_a, player_b], world_map


def army_headquarters_scenario(general_a, general_b):
    """Scénario de siège
    Armée A: Défenseur
    Armée B: attaquante"""
    ...
    # La MERVEILLE (Tout au fond, à gauche)
    spawn_unit_safe(UnitType.WONDER, 20*TILE, 60*TILE, player_a, world_map, all_units)
    
    # Les CROSSBOWMENS
    spawn_line(player_a, world_map, all_units, UnitType.ARCHER, 35*TILE, 50*TILE, 8, TILE)
    
    # Les PIKEMENS
    spawn_line(player_a, world_map, all_units, UnitType.PIKEMAN, 45*TILE, 45*TILE, 12, TILE)


    # Knights qui arrive de la droite
    spawn_square(player_b, world_map, all_units, UnitType.KNIGHT, 80*TILE, 50*TILE, 5, 6, TILE, 30)
...
SCENARIOS = {
    ...
    "mixed": mixed_battle_scenario,
    "headquarters": army_headquarters_scenario, 
}
